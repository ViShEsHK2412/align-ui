...
import { nest, SEMANTIC, surfaceShadow, themed, TYPE } from './theme';
...
@media (prefers-color-scheme: dark) {
  .picker { box-shadow: ${surfaceShadow(4, true)}; }
}
...
.picker button:hover { background: ${nest(2)}; }
.picker button:focus-visible { outline: 1px solid ${themed(SEMANTIC.fg)}; outline-offset: -1px; }
.picker .k { color: ${themed(SEMANTIC.muted)}; }
.picker .v { overflow-wrap: anywhere; }
.picker .hint {
  margin-top: 6px; padding-top: 6px;
  border-top: 1px solid color-mix(in oklab, ${themed(SEMANTIC.fg)} 12%, transparent);
  color: ${themed(SEMANTIC.muted)};
}
`;
