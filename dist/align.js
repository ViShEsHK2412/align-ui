...
`,te=O,j=-1,ne=!1;function De(e){let n=document.createElement("style");n.textContent=vt,e.appendChild(n);let o=document.createElement("div");o.className="dock";let t=document.createElement("div");t.className="panel",o.appendChild(t),e.appendChild(o);let r=(s,v)=>Math.min(Math.max(s,O),Math.max(O,v-O));function i(){let s=o.offsetHeight||300;j<0&&(j=Math.max(O,innerHeight-s-O)),te=r(te,innerWidth-o.offsetWidth),j=r(j,innerHeight-s),o.style.transform=`translate(${te-O}px, ${j}px)`}let a=null;function d(s){s.button===0&&(s.preventDefault(),s.stopPropagation(),a={x:s.clientX,y:s.clientY,dx:te,dy:j},o.setAttribute("data-dragging",""),s.currentTarget.setPointerCapture(s.pointerId))}function L(s){a&&(te=a.dx+(s.clientX-a.x),j=a.dy+(s.clientY-a.y),i())}function P(){a=null,o.removeAttribute("data-dragging")}addEventListener("resize",i);let X=null;function N(s){let v=document.createElement("div");return v.className="edge",v.textContent=s===0?"0":f(s),s===0&&v.setAttribute("data-zero",""),v}function J(s,v,Z,z){let[l,c,y,W]=Z,B=document.createElement("div");B.className="region",B.setAttribute("data-level",String(v));let u=document.createElement("span");u.className="tag",u.textContent=s;let p=document.createElement("div");p.className="row";let E=document.createElement("div");return E.className="fill",E.appendChild(z),p.append(N(W),E,N(c)),B.append(u,N(l),p,N(y)),B}return{show(s){let v=Le(s.el),[Z,z,l,c]=v.border,[y,W,B,u]=v.padding,p=Pe(s.el),E=s.width/p.x,oe=s.height/p.y,re=Math.abs(p.x-1)>.001||Math.abs(p.y-1)>.001,A=document.createElement("header"),he=document.createElement("span");he.className="name",he.textContent=s.label;let me=document.createElement("span");me.className="size",me.textContent=`${f(E)} \xD7 ${f(oe)}`;let U=document.createElement("button");if(U.className="close",U.textContent="\xD7",U.title="close (B brings it back)",U.addEventListener("pointerdown",H=>H.stopPropagation()),U.addEventListener("click",H=>{H.stopPropagation(),ne=!0,o.removeAttribute("data-open")}),A.append(he,me),re){let H=document.createElement("span");H.className="scale",H.textContent=`\xD7${f(p.x)}`,H.title=`renders at ${f(s.width)} \xD7 ${f(s.height)}`,A.appendChild(H)}A.appendChild(U),A.addEventListener("pointerdown",d),A.addEventListener("pointermove",L),A.addEventListener("pointerup",P),A.addEventListener("pointercancel",P);let fe=document.createElement("div");fe.className="content",fe.textContent=`${f(E-c-z-u-W)} \xD7 ${f(oe-Z-l-y-B)}`,t.replaceChildren(A,J("margin",1,v.margin,J("border",2,v.border,J("padding",3,v.padding,fe)))),X=s,i(),!ne&&requestAnimationFrame(()=>o.setAttribute("data-open",""))},hide(){X=null,o.removeAttribute("data-open")},toggle(){X&&(ne=!ne,ne?o.removeAttribute("data-open"):(i(),o.setAttribute("data-open","")))},destroy(){removeEventListener("resize",i),o.remove(),n.remove()}}}var bt=[["Ctrl/Cmd + Shift + A","turn align on or off"],["Hover","measure what is under the cursor"],["Click","lock an element"],["Right-click","add to, or drop from, the locked set"],["Drag the panel header","move the box model"],["B","hide or bring back the box model"],["R","rulers down the top and left edges"],["Drag from a rule","pull out a guide; drag it back to remove"],["V","vertical guide at the cursor"],["H","horizontal guide at the cursor"],["Hover a guide","distance from it to every locked element"],["Click a guide","keep those distances up; click again to release"],["Alt while placing","ignore snapping"],["Del","remove the guide under the cursor; Shift+Del for all"],["Esc","clear the locks, then close"]],kt=`
.flag {
...
.help kbd {
  ...
  border: 1px solid color-mix(in oklab, ${R(G.fg)} 14%, transparent);
  background: ${T(2)};
}
.help dd { margin: 0; align-self: center; color: ${R(G.muted)}; }
`;function ze(e){let n=document.createElement("style");n.textContent=kt,e.appendChild(n);let o=document.createElement("div");o.className="flag";let t=document.createElement("span");t.className="name",t.textContent="Align";let r=document.createElement("span");r.className="count",o.append(t,r);let i=document.createElement("div");i.className="help";let a=document.createElement("dl");for(let[d,L]of bt){let P=document.createElement("dt"),X=document.createElement("kbd");X.textContent=d,P.appendChild(X);let N=document.createElement("dd");N.textContent=L,a.append(P,N)}return i.appendChild(a),o.addEventListener("click",d=>{d.stopPropagation(),i.toggleAttribute("data-open")}),e.append(o,i),{update(d){r.textContent=d>0?`${d} locked`:""},closeHelp(){let d=i.hasAttribute("data-open");return i.removeAttribute("data-open"),d},destroy(){o.remove(),i.remove(),n.remove()}}}var se=5,Ee=4,ce=12,h=22,q=10,wt=50,Et=100;function He(){let e=document.createElement("div");e.id="__align_host",e.setAttribute("data-align-ignore",""),e.style.cssText="all: initial; position: fixed; inset: 0; z-index: 2147483647; pointer-events: none;",document.documentElement.appendChild(e);let n=e.attachShadow({mode:"closed"}),o=document.createElement("canvas");o.style.cssText="position: fixed; inset: 0; pointer-events: none;",n.appendChild(o);let t=o.getContext("2d"),r={hover:null,pinned:[],lines:[],cursor:null,rulers:!1,guides:[],liveGuide:null},i=ke(Ne()),a=0,d=matchMedia("(prefers-color-scheme: dark)"),L=()=>{i=ke(d.matches),z()};d.addEventListener("change",L),Xe(()=>z());function P(){let l=devicePixelRatio;o.width=Math.round(innerWidth*l),o.height=Math.round(innerHeight*l),o.style.width=innerWidth+"px",o.style.height=innerHeight+"px",t.setTransform(l,0,0,l,0,0),t.translate(.5,.5)}function X(l,c){t.strokeStyle=c,t.lineWidth=1,t.setLineDash([]),t.strokeRect(Math.round(l.left),Math.round(l.top),Math.round(l.width),Math.round(l.height))}function N(l){t.strokeStyle=ae(i.measure,.7),t.lineWidth=1,t.setLineDash([2,2]),t.beginPath();for(let c of[l.left,l.right])t.moveTo(Math.round(c),0),t.lineTo(Math.round(c),innerHeight);for(let c of[l.top,l.bottom])t.moveTo(0,Math.round(c)),t.lineTo(innerWidth,Math.round(c));t.stroke(),t.setLineDash([])}function J(l){if(t.strokeStyle=i.measure,t.lineWidth=1,t.setLineDash([]),t.beginPath(),t.moveTo(Math.round(l.x1),Math.round(l.y1)),t.lineTo(Math.round(l.x2),Math.round(l.y2)),l.axis==="x")for(let c of[l.x1,l.x2])t.moveTo(Math.round(c),Math.round(l.y1)-se),t.lineTo(Math.round(c),Math.round(l.y1)+se);else for(let c of[l.y1,l.y2])t.moveTo(Math.round(l.x1)-se,Math.round(c)),t.lineTo(Math.round(l.x1)+se,Math.round(c));t.stroke()}function s(l,c,y,W,B=!1){t.font=`${k.medium} ${g.body}px ${g.stack}`,t.textBaseline="middle";let u=t.measureText(l).width+Ee*2,p=g.body+Ee*2+2,E=B?c-u/2:c,oe=B?y-p/2:y,re=Math.min(Math.max(E,ce),innerWidth-u-ce),A=Math.min(Math.max(oe,ce),innerHeight-p-ce);t.fillStyle=W,t.beginPath(),t.roundRect(re,A,u,p,4),t.fill(),t.fillStyle=i.surface,t.fillText(l,re+Ee,A+p/2)}function v(){let l=scrollX,c=scrollY;t.fillStyle=i.rulerBg,t.fillRect(-.5,-.5,innerWidth+1,h),t.fillRect(-.5,-.5,h,innerHeight+1),t.strokeStyle=i.rulerLine,t.lineWidth=1,t.setLineDash([]),t.font=`${k.regular} 9px ${g.stack}`,t.fillStyle=i.muted,t.save(),t.globalAlpha=.16,t.fillStyle=i.accent;for(let u of r.pinned)t.fillRect(u.left,-.5,u.width,h),t.fillRect(-.5,u.top,h,u.height);t.restore(),t.beginPath(),t.moveTo(-.5,h-.5),t.lineTo(innerWidth,h-.5),t.moveTo(h-.5,-.5),t.lineTo(h-.5,innerHeight),t.stroke();let y=u=>u%Et===0?h:u%wt===0?7:4;t.textBaseline="top",t.textAlign="left",t.beginPath();let W=Math.floor(l/q)*q;for(let u=W;u<l+innerWidth;u+=q){let p=Math.round(u-l);if(p<h)continue;let E=y(u);t.moveTo(p,h-E),t.lineTo(p,h),E===h&&(t.fillStyle=i.muted,t.fillText(String(u),p+3,3))}t.stroke(),t.beginPath();let B=Math.floor(c/q)*q;for(let u=B;u<c+innerHeight;u+=q){let p=Math.round(u-c);if(p<h)continue;let E=y(u);t.moveTo(h-E,p),t.lineTo(h,p),E===h&&(t.save(),t.translate(3,p-3),t.rotate(-Math.PI/2),t.fillStyle=i.muted,t.fillText(String(u),0,0),t.restore())}t.stroke(),r.cursor&&(t.strokeStyle=i.accent,t.beginPath(),t.moveTo(Math.round(r.cursor.x),-.5),t.lineTo(Math.round(r.cursor.x),h),t.moveTo(-.5,Math.round(r.cursor.y)),t.lineTo(h,Math.round(r.cursor.y)),t.stroke()),t.fillStyle=i.guide;for(let u of r.guides){let p=Math.round(ee(u));u.axis==="x"?t.fillRect(p-1,-.5,2,h):t.fillRect(-.5,p-1,h,2)}t.fillStyle=i.rulerBg,t.fillRect(-.5,-.5,h,h),t.strokeStyle=i.rulerLine,t.strokeRect(-.5,-.5,h,h)}function Z(){a=0,t.save(),t.setTransform(1,0,0,1,0,0),t.clearRect(0,0,o.width,o.height),t.restore();for(let l of r.pinned)X(l,i.accent);r.hover&&(N(r.hover),X(r.hover,r.pinned.length?ae(i.accent,.7):i.accent));for(let l of r.guides){let c=r.liveGuide?.id===l.id;t.strokeStyle=l.locked||c?i.guide:ae(i.guide,.55),t.lineWidth=1,t.setLineDash(l.locked?[]:[4,4]),t.beginPath();let y=Math.round(ee(l));l.axis==="x"?(t.moveTo(y,0),t.lineTo(y,innerHeight)):(t.moveTo(0,y),t.lineTo(innerWidth,y)),t.stroke()}for(let l of r.lines)J(l);for(let l of r.lines){let c=(l.x1+l.x2)/2,y=(l.y1+l.y2)/2;l.axis==="x"?s(l.label,c,y-16,i.measure,!0):s(l.label,c+26,y,i.measure,!0)}if(r.hover&&r.cursor){let{width:l,height:c}=r.hover;s(`${f(l)} \xD7 ${f(c)}`,r.cursor.x+14,r.cursor.y+14,i.accent)}if(r.liveGuide){let l=r.liveGuide,c=Math.round(ee(l));s(`${l.axis} ${f(l.at)}`,l.axis==="x"?c+6:30,l.axis==="x"?30:c+6,i.guide)}r.rulers&&v()}function z(){a||(a=requestAnimationFrame(Z))}return P(),{root:n,update(l){Object.assign(r,l),z()},resize(){P(),z()},destroy(){a&&cancelAnimationFrame(a),d.removeEventListener("change",L),e.remove()}}}var S,M=null,I=null,F=null,x=null,m=[],ue=0,pe=!1,w=[],Mt=1,$=null,D=null,Y=null,$t=3,Q=22;function je(e,n){return pe?n<Q&&e>=Q?"y":e<Q&&n>=Q?"x":null:null}function Ue(e,n,o,t){let r=_(n,o,S),i=e.axis==="x"?n:o,a=Ae(i,Re(r,e.axis),t);e.at=a+(e.axis==="x"?scrollX:scrollY)}function qe(e,n,o,t){let r={id:Mt++,axis:e,at:0,locked:!1};return Ue(r,n,o,t),w=[...w,r],r}function Qe(e){w=w.filter(n=>n.id!==e.id),D?.id===e.id&&(D=null),$?.id===e.id&&($=null)}function St(e){let n=S.hotkey.toLowerCase().split("+"),o=n[n.length-1];return e.key.toLowerCase()!==o||n.includes("shift")!==e.shiftKey||n.includes("alt")!==e.altKey?!1:(n.includes("mod")||n.includes("ctrl")||n.includes("cmd"))===(e.metaKey||e.ctrlKey)}function Ke(e){return{axis:e.axis,pos:e.axis==="x"?e.at-scrollX:e.at-scrollY}}function C(e){let n=m[m.length-1],o=x&&m.some(a=>a.el===x.el),t=w.map(Ke),r=!$&&D?D:null,i=w.filter(a=>a.locked||a.id===r?.id);M?.update({hover:x,pinned:m,rulers:pe,guides:w,liveGuide:$??D,lines:[...Be(m),...n&&x&&!o&&!r?ge(n,x):[],...i.flatMap(a=>m.flatMap(d=>ye(d,[Ke(a)]))),...x&&!r&&w.length?ye(x,t):[]],...e?{cursor:e}:{}}),F?.update(m.length)}var de=null;function Ve(e){if(de={x:e.clientX,y:e.clientY},$){Y&&Math.hypot(e.clientX-Y.x,e.clientY-Y.y)>$t&&(Y=null),Y||(Ue($,e.clientX,e.clientY,e.altKey),w=[...w]),C({x:e.clientX,y:e.clientY});return}D=xe(w,e.clientX,e.clientY),x=_(e.clientX,e.clientY,S),C({x:e.clientX,y:e.clientY})}function Je(e){$&&(Y?($.locked=!$.locked,w=[...w]):(je(e.clientX,e.clientY)||e.clientX<Q||e.clientY<Q)&&Qe($),Y=null,$=null,C({x:e.clientX,y:e.clientY}))}function Ze(e){if(e.button!==0)return;let n=_(e.clientX,e.clientY,S);if(!n)return;let o=je(e.clientX,e.clientY);if(o){V(e),Y=null,$=qe(o,e.clientX,e.clientY,e.altKey),C({x:e.clientX,y:e.clientY});return}let t=xe(w,e.clientX,e.clientY);if(t){V(e),$=t,Y={x:e.clientX,y:e.clientY},C({x:e.clientX,y:e.clientY});return}V(e),F?.closeHelp(),m=[n],x=n,I?.show(n),C({x:e.clientX,y:e.clientY})}function et(e){let n=_(e.clientX,e.clientY,S);if(!n)return;V(e),F?.closeHelp();let o=m.findIndex(r=>r.el===n.el);m=o>=0?m.filter((r,i)=>i!==o):[...m,n],x=n;let t=m[m.length-1];t?I?.show(t):I?.hide(),C({x:e.clientX,y:e.clientY})}function tt(e){_(e.clientX,e.clientY,S)&&V(e)}function nt(e){_(e.clientX,e.clientY,S)&&V(e)}function V(e){e.preventDefault(),e.stopPropagation()}function Oe(e,n){return e.left===n.left&&e.top===n.top&&e.width===n.width&&e.height===n.height}var Fe=0,We=0;function ot(){ue=requestAnimationFrame(ot);let n=m.filter(a=>a.el.isConnected).map(a=>le(a.el)),o=x&&x.el.isConnected?le(x.el):null;if(!(scrollX!==Fe||scrollY!==We||n.length!==m.length||n.some((a,d)=>!Oe(a,m[d]))||x===null!=(o===null)||x!==null&&o!==null&&!Oe(x,o)))return;Fe=scrollX,We=scrollY,m=n,x=o;let i=m[m.length-1];i?I?.show(i):I?.hide(),C()}function rt(){M?.resize()}function Ct(){M||(Ye(),M=He(),I=De(M.root),F=ze(M.root),F.update(0),addEventListener("mousemove",Ve),addEventListener("mousedown",Ze,{capture:!0}),addEventListener("mouseup",Je,{capture:!0}),addEventListener("click",tt,{capture:!0}),addEventListener("auxclick",nt,{capture:!0}),addEventListener("contextmenu",et,{capture:!0}),addEventListener("resize",rt),ue=requestAnimationFrame(ot),C())}function Me(){removeEventListener("mousemove",Ve),removeEventListener("mousedown",Ze,{capture:!0}),removeEventListener("mouseup",Je,{capture:!0}),removeEventListener("click",tt,{capture:!0}),removeEventListener("auxclick",nt,{capture:!0}),removeEventListener("contextmenu",et,{capture:!0}),removeEventListener("resize",rt),cancelAnimationFrame(ue),ue=0,F?.destroy(),F=null,I?.destroy(),I=null,M?.destroy(),M=null,Ie(),x=null,m=[],$=null,Y=null,D=null}function _e(e){if(St(e))e.preventDefault(),M?Me():Ct();else if(M&&de&&(e.key.toLowerCase()===S.guideKeys.vertical||e.key.toLowerCase()===S.guideKeys.horizontal)){e.preventDefault();let n=e.key.toLowerCase()===S.guideKeys.vertical?"x":"y";qe(n,de.x,de.y,e.altKey),C()}else if(M&&(e.key==="Delete"||e.key==="Backspace"))e.preventDefault(),e.shiftKey?w=[]:D&&Qe(D),C();else if(M&&e.key.toLowerCase()===S.rulerKey)e.preventDefault(),pe=!pe,C();else if(M&&e.key.toLowerCase()===S.panelKey)e.preventDefault(),I?.toggle();else if(e.key==="Escape"&&M){if(F?.closeHelp())return;m.length?(m=[],I?.hide(),C()):Me()}}function _t(e={}){if(typeof window>"u"||window.__align)return;window.__align=!0,S=Se(e),addEventListener("keydown",_e,{capture:!0});let n=import.meta.hot;n&&n.dispose(()=>{Me(),removeEventListener("keydown",_e,{capture:!0}),delete window.__align})}export{_t as initAlign};
