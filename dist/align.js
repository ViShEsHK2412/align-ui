...
`,pe=J,Z=-1,me=!1;function Et(e){let t=document.createElement("style");t.textContent=wn,e.appendChild(t);let o=document.createElement("div");o.className="dock";let n=document.createElement("div");n.className="panel",o.appendChild(n);function r(u,E){let S=document.createElement("div");S.className="readout";let P=document.createElement("div");P.className="tag readout-tag",P.textContent=u,S.appendChild(P);for(let[R,O]of E){let l=document.createElement("div");l.className="readout-row";let p=document.createElement("span");p.className="readout-key",p.textContent=R;let c=document.createElement("span");c.className="readout-value",c.textContent=O,l.append(p,c),S.appendChild(l)}return S}e.appendChild(o);let i=(u,E)=>Math.min(Math.max(u,J),Math.max(J,E-J));function a(){let u=o.offsetHeight||300;Z<0&&(Z=Math.max(J,innerHeight-u-J)),pe=i(pe,innerWidth-o.offsetWidth),Z=i(Z,innerHeight-u),o.style.transform=`translate(${pe-J}px, ${Z}px)`}let s=null;function h(u){u.button===0&&(u.preventDefault(),u.stopPropagation(),s={x:u.clientX,y:u.clientY,dx:pe,dy:Z},o.setAttribute("data-dragging",""),u.currentTarget.setPointerCapture(u.pointerId))}function M(u){s&&(pe=s.dx+(u.clientX-s.x),Z=s.dy+(u.clientY-s.y),a())}function x(){s=null,o.removeAttribute("data-dragging")}addEventListener("resize",a);let N=null;function m(u){let E=document.createElement("div");return E.className="edge",E.textContent=u===0?"0":v(u),u===0&&E.setAttribute("data-zero",""),E}function G(u,E,S,P){let[R,O,l,p]=S,c=document.createElement("div");c.className="region",c.setAttribute("data-level",String(E));let w=document.createElement("span");w.className="tag",w.textContent=u;let f=document.createElement("div");f.className="row";let d=document.createElement("div");d.className="fill",d.appendChild(P),f.append(m(p),d,m(O));let g=document.createElement("div");return g.className="head",g.append(w,m(R)),c.append(g,f,m(l)),c}return{show(u,E=[]){let S=dt(u.el),[P,R,O,l]=S.border,[p,c,w,f]=S.padding,d=gt(u.el),g=u.width/d.x,D=u.height/d.y,nn=Math.abs(d.x-1)>.001||Math.abs(d.y-1)>.001,j=document.createElement("header"),Te=document.createElement("span");Te.className="name",Te.textContent=u.label;let Me=document.createElement("span");Me.className="size",Me.textContent=`${v(g)} \xD7 ${v(D)}`;let te=document.createElement("button");if(te.className="close",te.textContent="\xD7",te.title="close (B brings it back)",te.addEventListener("pointerdown",$=>$.stopPropagation()),te.addEventListener("click",$=>{$.stopPropagation(),me=!0,o.removeAttribute("data-open")}),j.append(Te,Me),nn){let $=document.createElement("span");$.className="scale",$.textContent=`\xD7${v(d.x)}`,$.title=`renders at ${v(u.width)} \xD7 ${v(u.height)}`,j.appendChild($)}j.appendChild(te),j.addEventListener("pointerdown",h),j.addEventListener("pointermove",M),j.addEventListener("pointerup",x),j.addEventListener("pointercancel",x);let Le=document.createElement("div");Le.className="content",Le.textContent=`${v(g-l-R-f-c)} \xD7 ${v(D-P-O-p-w)}`;let ne=[j,G("margin",1,S.margin,G("border",2,S.border,G("padding",3,S.padding,Le)))];if(E.length){let $=E.map(U=>[v(U.px),U.detail]),ye=nt(E.map(U=>U.px));ye&&$.push(["",ye]),ne.push(r("gaps",$))}let Xe=Ve(u.el),Ke=Ze([g,D,...S.margin,...S.border,...S.padding],Xe);Ke&&ne.push(r("tokens",[["",Ke]]));let _e=at(u.el);_e.length&&ne.push(r("styled by",_e.slice(0,4).map($=>[$.selector,$.file])));let je=it(u.el);je>1&&ne.push(r("matches",[["",`${je} elements share ${Ae(u.el)}`]]));let Ue=Xe.filter($=>Re($.value));if(Ue.length){let $=rt(u.el).map(({label:ye,value:U})=>{let qe=ot(U,Ue);return[ye,qe.length?`${U}  ${qe.join(" ")}`:`${U}  \u2014`]});$.length&&ne.push(r("colour",$))}n.replaceChildren(...ne),N=u,a(),!me&&requestAnimationFrame(()=>o.setAttribute("data-open",""))},hide(){N=null,o.removeAttribute("data-open")},toggle(){N&&(me=!me,me?o.removeAttribute("data-open"):(a(),o.setAttribute("data-open","")))},destroy(){removeEventListener("resize",a),o.remove(),t.remove()}}}var kn=[["Ctrl/Cmd + Shift + A","turn align on or off"],["Hover","measure what is under the cursor"],["Click","lock an element"],["Right-click","add to, or drop from, the locked set"],["Drag the panel header","move the box model"],["B","hide or bring back the box model"],["R","rulers down the top and left edges"],["Drag from a rule","pull out a guide; drag it back to remove"],["V","vertical guide at the cursor"],["H","horizontal guide at the cursor"],["Hover a guide","distance from it to every locked element"],["Click a guide","keep those distances up; click again to release"],["Arrows","nudge the last guide you touched; Shift for 10px"],["L","pin that guide so it cannot be moved or deleted"],["Ctrl/Cmd while placing","ignore snapping"],["Del","remove the guide under the cursor; Shift+Del for all"],["Esc","clear the locks, then close"]],fe=16,$t=k.tag+12,Ct=8,Sn=`
.flag {
