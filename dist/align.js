...
.help[data-rulers] { top: ${ie+k+De+Pe}px; }
...
`,wt=[{name:"rulers",label:"Rulers",key:"R",toggle:!0,what:"a scale down the top and left edges, in page pixels \u2014 and the gutters you drag guides out of"},{name:"xray",label:"X-ray",key:"X",toggle:!0,what:"outline every element at once, to see the boxes a layout is really made of"},{name:"grid",label:"Column grid",key:"G",toggle:!0,what:"the grid your design is built on, columns filled and gutters left empty. Needs one configured"},{name:"pixels",label:"Pixel grid",key:"K",toggle:!0,what:"a ten-pixel lattice over the page, to read an offset off without measuring it"},{name:"type",label:"Type",key:"T",toggle:!0,what:"add size, weight, line height and tracking to the panel, each checked against your scale"},{name:"panel",label:"Box model",key:"B",toggle:!0,what:"the readout itself \u2014 margins, borders, padding, what places the element, what styles it"},{name:"freeze",label:"Freeze",key:"F",toggle:!0,what:"hold every animation and transition where it stands, so a moving thing can be measured"},{name:"copy",label:"Copy",key:"C",toggle:!1,what:"put everything in the panel on the clipboard as text"},{name:"pick",label:"Colour",key:"P",toggle:!1,what:"sample a colour from anywhere on screen and match it against your palette"},{name:"undo",label:"Undo",key:"Ctrl/Cmd + Z",toggle:!1,what:"step back through the guides \u2014 a whole run of nudges counts as one"}];function Sn(e,t){let o=document.createElement("style");o.textContent=Ao,e.appendChild(o);let n=document.createElement("div");n.className="flag";let r=document.createElement("span");r.className="name",r.textContent="Align";let i=document.createElement("span");i.className="count";let a=new Map,s=new Map,c=document.createElement("div");c.className="tools";for(let u of wt){if(u.name==="freeze"||u.name==="copy"){let G=document.createElement("span");G.className="sep",c.appendChild(G)}let f=document.createElement("button");f.type="button",f.className="tool";let w=Ve(u.name);w.classList.add("glyph"),f.appendChild(w),f.setAttribute("aria-label",u.label),f.title=`${u.label}  \xB7  ${u.key}
${u.what}`,u.toggle||f.setAttribute("data-once",""),f.addEventListener("click",G=>{G.stopPropagation(),t(u.name)}),a.set(u.name,f),c.appendChild(f)}n.append(r,c,i);let m=document.createElement("div");m.className="help";let g=document.createElement("dl");function $(u){let f=document.createElement("h4");f.textContent=u,g.appendChild(f)}function p(u,f,w){let G=document.createElement("span");G.className="glyph",w&&G.appendChild(Ve(w,14));let d=document.createElement("dt"),b=document.createElement("kbd");b.textContent=u,d.appendChild(b);let M=document.createElement("dd");M.textContent=f,g.append(G,d,M)}$("The bar, left to right");for(let u of wt)p(u.key,`${u.label} \u2014 ${u.what}`,u.name);for(let u of Lo){$(u.title);for(let[f,w]of u.rows)p(f,w)}return m.appendChild(g),n.addEventListener("click",u=>{u.stopPropagation(),m.toggleAttribute("data-open")}),e.append(n,m),{acknowledge(u,f){let w=a.get(u);if(!w)return;clearTimeout(s.get(u)),w.querySelector(".ack")?.remove();let G=Ve(f?"check":"cross");G.classList.add("ack"),w.appendChild(G),requestAnimationFrame(()=>w.setAttribute("data-ack",f?"yes":"no")),s.set(u,setTimeout(()=>{w.removeAttribute("data-ack"),setTimeout(()=>w.querySelector(".ack")?.remove(),200)},Ro))},update(u,f){i.textContent=u>0?`${u} locked`:"",n.toggleAttribute("data-rulers",f.rulers),m.toggleAttribute("data-rulers",f.rulers);for(let w of wt)w.toggle&&a.get(w.name)?.toggleAttribute("data-on",f[w.name]===!0)},closeHelp(){let u=m.hasAttribute("data-open");return m.removeAttribute("data-open"),u},destroy(){for(let u of s.values())clearTimeout(u);n.remove(),m.remove(),o.remove()}}}var qe=5,vt=4,Fe=12,En=.22,Ce=10,Go=50,No=100;function Cn(){let e=document.createElement("div");e.id="__align_host",e.setAttribute("data-align-ignore",""),e.style.cssText="all: initial; position: fixed; inset: 0; z-index: 2147483647; pointer-events: none;",document.documentElement.appendChild(e);let t=e.attachShadow({mode:"closed"}),o=document.createElement("canvas");o.style.cssText="position: fixed; inset: 0; pointer-events: none;",t.appendChild(o);let n=o.getContext("2d"),r={hover:null,pinned:[],lines:[],cursor:null,rulers:!1,grid:null,pixels:!1,guides:[],liveGuide:null,activeGuide:null},i=gt(xt()),a=0,s=null;function c(){let l=xt();l!==s&&(s=l,i=gt(l),e.style.colorScheme=l?"dark":"light",D())}c();let m=matchMedia("(prefers-color-scheme: dark)"),g=()=>c();m.addEventListener("change",g);let $=new MutationObserver(()=>c());function p(){$.disconnect(),$.observe(document.documentElement,{attributes:!0}),document.body&&$.observe(document.body,{attributes:!0})}p(),vn(()=>D());function u(){let l=devicePixelRatio;o.width=Math.round(innerWidth*l),o.height=Math.round(innerHeight*l),o.style.width=innerWidth+"px",o.style.height=innerHeight+"px",n.setTransform(l,0,0,l,0,0),n.translate(.5,.5)}let f=l=>Math.round(l)-.5;function w(l,x){n.strokeStyle=x,n.lineWidth=1,n.setLineDash([]),n.strokeRect(Math.round(l.left),Math.round(l.top),Math.round(l.width),Math.round(l.height))}function G(l){n.strokeStyle=Se(i.measure,.7),n.lineWidth=1,n.setLineDash([2,2]),n.beginPath();for(let x of[l.left,l.right])n.moveTo(Math.round(x),0),n.lineTo(Math.round(x),innerHeight);for(let x of[l.top,l.bottom])n.moveTo(0,Math.round(x)),n.lineTo(innerWidth,Math.round(x));n.stroke(),n.setLineDash([])}function d(l){if(n.strokeStyle=l.extension?Se(i.measure,.55):i.measure,n.lineWidth=1,n.setLineDash(l.extension?[3,3]:[]),n.beginPath(),n.moveTo(Math.round(l.x1),Math.round(l.y1)),n.lineTo(Math.round(l.x2),Math.round(l.y2)),l.extension){n.stroke();return}if(l.axis==="x")for(let x of[l.x1,l.x2])n.moveTo(Math.round(x),Math.round(l.y1)-qe),n.lineTo(Math.round(x),Math.round(l.y1)+qe);else for(let x of[l.y1,l.y2])n.moveTo(Math.round(l.x1)-qe,Math.round(x)),n.lineTo(Math.round(l.x1)+qe,Math.round(x));n.stroke()}function b(l){return n.font=`${W.medium} ${T.body}px ${T.stack}`,{w:n.measureText(l).width+vt*2,h:T.body+vt*2+2}}function M(l,x,h,L){n.font=`${W.medium} ${T.body}px ${T.stack}`,n.textBaseline="middle";let{w:v,h:y}=b(l),N=f(Math.min(Math.max(x,Fe),innerWidth-v-Fe)),Y=f(Math.min(Math.max(h,Fe),innerHeight-y-Fe));n.fillStyle=L,n.beginPath(),n.roundRect(N,Y,Math.ceil(v),y,4),n.fill(),n.fillStyle=i.surface,n.fillText(l,N+vt,Y+y/2)}function B(l,x,h,L,v=!1){let{w:y,h:N}=b(l);M(l,v?x-y/2:x,v?h-N/2:h,L)}function H(){let l=scrollX,x=scrollY;n.fillStyle=i.rulerBg,n.fillRect(-.5,-.5,innerWidth+1,k),n.fillRect(-.5,-.5,k,innerHeight+1),n.strokeStyle=i.rulerLine,n.lineWidth=1,n.setLineDash([]),n.font=`${W.regular} 9px ${T.stack}`,n.fillStyle=i.muted,n.save(),n.globalAlpha=.16,n.fillStyle=i.accent;for(let y of r.pinned)n.fillRect(f(y.left),-.5,Math.round(y.width),k),n.fillRect(-.5,f(y.top),k,Math.round(y.height));n.restore(),n.beginPath(),n.moveTo(-.5,k-.5),n.lineTo(innerWidth,k-.5),n.moveTo(k-.5,-.5),n.lineTo(k-.5,innerHeight),n.stroke();let h=y=>y%No===0?k:y%Go===0?7:4;n.textBaseline="top",n.textAlign="left",n.beginPath();let L=Math.floor(l/Ce)*Ce;for(let y=L;y<l+innerWidth;y+=Ce){let N=Math.round(y-l);if(N<k)continue;let Y=h(y);n.moveTo(N,k-Y),n.lineTo(N,k),Y===k&&(n.fillStyle=i.muted,n.fillText(String(y),N+3,3))}n.stroke(),n.beginPath();let v=Math.floor(x/Ce)*Ce;for(let y=v;y<x+innerHeight;y+=Ce){let N=Math.round(y-x);if(N<k)continue;let Y=h(y);n.moveTo(k-Y,N),n.lineTo(k,N),Y===k&&(n.save(),n.translate(3,N-3),n.rotate(-Math.PI/2),n.fillStyle=i.muted,n.fillText(String(y),0,0),n.restore())}n.stroke(),r.cursor&&(n.strokeStyle=i.accent,n.beginPath(),n.moveTo(Math.round(r.cursor.x),-.5),n.lineTo(Math.round(r.cursor.x),k),n.moveTo(-.5,Math.round(r.cursor.y)),n.lineTo(k,Math.round(r.cursor.y)),n.stroke()),n.fillStyle=i.guide;for(let y of r.guides){let N=Math.round(Be(y));y.axis==="x"?n.fillRect(N-1,-.5,2,k):n.fillRect(-.5,N-1,k,2)}n.fillStyle=i.rulerBg,n.fillRect(-.5,-.5,k,k),n.strokeStyle=i.rulerLine,n.strokeRect(-.5,-.5,k,k)}function Q(){let l=fn(10,1);if(l){n.strokeStyle=i.pixelLine,n.lineWidth=1,n.setLineDash([]),n.beginPath();for(let x=0;x<=innerWidth;x+=l)n.moveTo(x,0),n.lineTo(x,innerHeight);for(let x=0;x<=innerHeight;x+=l)n.moveTo(0,x),n.lineTo(innerWidth,x);n.stroke()}}function j(l){let x=mn(l,document.documentElement.clientWidth);n.fillStyle=Se(i.measure,.08);for(let h of x)n.fillRect(f(h.left),-.5,Math.round(h.width),innerHeight+1)}function K(){a=0,n.save(),n.setTransform(1,0,0,1,0,0),n.clearRect(0,0,o.width,o.height),n.restore(),(r.pixels||r.grid)&&(n.save(),r.rulers&&(n.beginPath(),n.rect(k,k,innerWidth,innerHeight),n.clip()),r.pixels&&Q(),r.grid&&j(r.grid),n.restore());for(let h of r.pinned)w(h,i.accent);r.hover&&(G(r.hover),w(r.hover,r.pinned.length?Se(i.accent,.7):i.accent));for(let h of r.guides){let L=r.liveGuide?.id===h.id;n.strokeStyle=h.locked||L?i.guide:Se(i.guide,.55),n.lineWidth=h.pinned?2:1,n.setLineDash(h.locked?[]:[4,4]),n.beginPath();let v=Math.round(Be(h));if(h.axis==="x"?(n.moveTo(v,0),n.lineTo(v,innerHeight)):(n.moveTo(0,v),n.lineTo(innerWidth,v)),n.stroke(),r.activeGuide===h.id){n.lineWidth=3,n.setLineDash([]),n.beginPath();let y=7;h.axis==="x"?(n.moveTo(v,0),n.lineTo(v,y),n.moveTo(v,innerHeight-y),n.lineTo(v,innerHeight)):(n.moveTo(0,v),n.lineTo(y,v),n.moveTo(innerWidth-y,v),n.lineTo(innerWidth,v)),n.stroke()}}for(let h of r.lines)n.globalAlpha=h.faded?En:1,d(h);n.globalAlpha=1;let l=r.lines.filter(h=>h.label!==""),x=l.map(h=>{let L=(h.x1+h.x2)/2,v=(h.y1+h.y2)/2,{w:y,h:N}=b(h.label);return h.axis==="x"?{x:L-y/2,y:v-16-N/2,w:y,h:N,axis:h.axis}:{x:L+26-y/2,y:v-N/2,w:y,h:N,axis:h.axis}});if(hn(x,{w:innerWidth,h:innerHeight},Fe).forEach((h,L)=>{let v=l[L];n.globalAlpha=v.faded?En:1,M(v.label,h.x,h.y,i.measure)}),n.globalAlpha=1,r.hover&&r.cursor){let{width:h,height:L,scale:v}=r.hover;B(`${R(h/v.x)} \xD7 ${R(L/v.y)}`,r.cursor.x+14,r.cursor.y+14,i.accent)}if(r.liveGuide){let h=r.liveGuide,L=Math.round(Be(h));B([`${h.axis} ${R(h.at)}`,h.caught,h.pinned?"pinned":""].filter(Boolean).join(" \xB7 "),h.axis==="x"?L+6:30,h.axis==="x"?30:L+6,i.guide)}r.rulers&&H()}function D(){a||(a=requestAnimationFrame(K))}return u(),{root:t,update(l){Object.assign(r,l),D()},resize(){u(),D()},destroy(){a&&cancelAnimationFrame(a),m.removeEventListener("change",g),$.disconnect(),e.remove()}}}function Bo(e){let t=e.trim().replace(/^#/,""),o=t.length===3?t.split("").map(n=>n+n).join(""):t;return/^[0-9a-f]{6}$/i.test(o)?{r:parseInt(o.slice(0,2),16),g:parseInt(o.slice(2,4),16),b:parseInt(o.slice(4,6),16)}:null}function Io({r:e,g:t,b:o}){let n=r=>Math.round(r).toString(16).padStart(2,"0");return`#${n(e)}${n(t)}${n(o)}`}function Oo({r:e,g:t,b:o}){return`rgb(${Math.round(e)} ${Math.round(t)} ${Math.round(o)})`}function be(e,t){return String(Number(e.toFixed(t)))}function Do({r:e,g:t,b:o}){let n=e/255,r=t/255,i=o/255,a=Math.max(n,r,i),s=Math.min(n,r,i),c=(a+s)/2,m=a-s,g=0,$=0;return m!==0&&($=m/(1-Math.abs(2*c-1)),a===n?g=(r-i)/m%6:a===r?g=(i-n)/m+2:g=(n-r)/m+4,g*=60,g<0&&(g+=360)),`hsl(${be(g,1)} ${be($*100,1)}% ${be(c*100,1)}%)`}function kt(e){let t=e/255;return t<=.04045?t/12.92:((t+.055)/1.055)**2.4}function Po(e){let t=kt(e.r),o=kt(e.g),n=kt(e.b),r=.4122214708*t+.5363325363*o+.0514459929*n,i=.2119034982*t+.6806995451*o+.1073969566*n,a=.0883024619*t+.2817188376*o+.6299787005*n,s=Math.cbrt(r),c=Math.cbrt(i),m=Math.cbrt(a),g=.2104542553*s+.793617785*c-.0040720468*m,$=1.9779984951*s-2.428592205*c+.4505937099*m,p=.0259040371*s+.7827717662*c-.808675766*m,u=Math.sqrt($*$+p*p),f=Math.atan2(p,$)*180/Math.PI;return f<0&&(f+=360),u<1e-4?`oklch(${be(g,4)} 0 0)`:`oklch(${be(g,4)} ${be(u,4)} ${be(f,2)})`}function Tn(e){let t=Bo(e);return t?[{label:"hex",value:Io(t)},{label:"rgb",value:Oo(t)},{label:"hsl",value:Do(t)},{label:"oklch",value:Po(t)}]:[]}var Fo=`
.picker {
  /* Under the badge, from the badge's own numbers. */
  position: fixed; top: ${ie+De+Pe}px; right: ${ie}px; width: 200px;
  padding: ${z.base}px; border-radius: 0;
  user-select: none;
  font-family: ${T.stack};
  font-variant-numeric: tabular-nums;
...
`;function Tt(e){let t=document.documentElement;if(!e){t.removeAttribute("data-align-xray"),document.getElementById(Ct)?.remove();return}if(!document.getElementById(Ct)){let o=document.createElement("style");o.id=Ct,o.textContent=zo,o.setAttribute("data-align-ignore",""),document.head.appendChild(o)}t.setAttribute("data-align-xray","")}var Mt="align-ui";function Rn(e){try{return localStorage.getItem(e)}catch{return null}}function An(e,t){try{localStorage.setItem(e,t)}catch{}}function Gn(e){let t="/";try{t=location.pathname||"/"}catch{}return`${Mt}:${e}::${t}`}function Wo(e){if(typeof e!="object"||e===null)return!1;let t=e;return(t.axis==="x"||t.axis==="y")&&typeof t.at=="number"&&Number.isFinite(t.at)}function Nn(){let e=Rn(Gn("guides"));if(!e)return[];try{let t=JSON.parse(e);return Array.isArray(t)?t.filter(Wo).map(o=>({id:0,axis:o.axis,at:o.at,locked:o.locked===!0,pinned:o.pinned===!0,caught:typeof o.caught=="string"?o.caught:""})):[]}catch{return[]}}function Bn(e){An(Gn("guides"),JSON.stringify(e.map(t=>({axis:t.axis,at:t.at,locked:t.locked,pinned:t.pinned,caught:t.caught}))))}function et(e){return Rn(`${Mt}:${e}`)==="1"}function tt(e,t){An(`${Mt}:${e}`,t?"1":"0")}var U,A=null,J=null,ue=null,ze=null,we=!1,Le=et("grid"),Re=et("pixels"),F=null,E=[],ot=0,ve=et("rulers"),O=[],Wn=1,In=!1,de=null,Te=$n();function Xo(){return O.map(e=>({...e}))}function Ae(e=""){Te.push(Xo(),e)}function On(){return O.find(e=>e.id===de)??null}function fe(e){O=e,Bn(O)}var X=null,re=null,te=null,Yo=3,Me=22;function Xn(e,t){return ve?t<Me&&e>=Me?"y":e<Me&&t>=Me?"x":null:null}function Rt(e){return e.ctrlKey||e.metaKey}function Yn(e,t,o,n){let r=ge(t,o,U),i=e.axis==="x"?t:o,a=O.filter(c=>c.id!==e.id).map(c=>({axis:c.axis,at:He(c).pos})),s=un(i,dn(r,e.axis,a),n);e.at=s.at+(e.axis==="x"?scrollX:scrollY),e.caught=s.what}function Kn(e,t,o,n){let r={id:Wn++,axis:e,at:0,locked:!1,caught:"",pinned:!1};return Ae(),Yn(r,t,o,n),fe([...O,r]),de=r.id,r}function _n(e){e.pinned||(Ae(),fe(O.filter(t=>t.id!==e.id)),re?.id===e.id&&(re=null),X?.id===e.id&&(X=null))}function Ko(e){let t=U.hotkey.toLowerCase().split("+"),o=t[t.length-1];return e.key.toLowerCase()!==o||t.includes("shift")!==e.shiftKey||t.includes("alt")!==e.altKey?!1:(t.includes("mod")||t.includes("ctrl")||t.includes("cmd"))===(e.metaKey||e.ctrlKey)}function He(e){return{axis:e.axis,pos:e.axis==="x"?e.at-scrollX:e.at-scrollY}}function At(){return E.length>=2?E[E.length-2]:void 0}function Gt(){if(E.length<2)return[];let e=[];for(let[t,o]of ut(E))for(let n of je(t,o)){if(n.extension||!n.label)continue;let r=Ut(t.el,o.el,parseFloat(n.label),n.axis);e.push({px:r.px,detail:Vt(r)})}return e}function V(e){let t=E[E.length-1],o=F&&E.some(p=>p.el===F.el),n=O.map(He),r=!X&&re?re:null,i=O.filter(p=>p.locked||p.id===r?.id),a=!r&&o?F.el:null,s=r??a,c=r?He(r):null,m=[],g=(p,u)=>{for(let f of p)m.push(s&&!u?{...f,faded:!0}:f)},$=p=>!c||p.axis!==c.axis?!1:(p.axis==="x"?[p.x1,p.x2]:[p.y1,p.y2]).some(f=>Math.abs(f-c.pos)<.5);for(let[p,u]of ut(E))g(je(p,u),p.el===a||u.el===a);t&&F&&!o&&!r&&g(je(t,F),!0);for(let p of i)for(let u of E)g(pt(u,[He(p)]),p.id===r?.id||u.el===a);F&&!o&&!r&&O.length&&g(pt(F,n),!0);for(let p of pn(i.map(He),{x:innerWidth/2,y:innerHeight/2}))g([p],$(p));A?.update({hover:F,pinned:E,rulers:ve,grid:Le&&U.grid?U.grid:null,pixels:Re,guides:O,liveGuide:X??re,activeGuide:de,lines:m,...e?{cursor:e}:{}}),ue?.update(E.length,{rulers:ve,xray:we,grid:Le,pixels:Re,freeze:Ze(),type:J?.showsType()??!1,panel:J?.isOpen()??!1})}function _o(){let e=J?.asText()??"";if(!e)return;let t=n=>ue?.acknowledge("copy",n),o=navigator.clipboard?.writeText(e);o?o.then(()=>t(!0),()=>t(!1)):t(!1)}function jo(e,t){return e.length===t.length&&e.every((o,n)=>{let r=t[n];return o.id===r.id&&o.axis===r.axis&&o.at===r.at&&o.locked===r.locked&&o.pinned===r.pinned})}function Uo(){for(;Te.depth()>0&&jo(Te.peek(),O);)Te.pop();let e=Te.pop();e&&(fe(e),re=null,X=null,te=null,e.some(t=>t.id===de)||(de=null))}function oe(e){switch(e){case"rulers":ve=!ve,tt("rulers",ve);break;case"xray":we=!we,Tt(we);break;case"grid":Le=!Le,tt("grid",Le);break;case"pixels":Re=!Re,tt("pixels",Re);break;case"freeze":Et(!Ze());break;case"type":J?.toggleType();break;case"panel":J?.toggle();break;case"copy":_o();break;case"pick":ze?.open();break;case"undo":Uo();break}V()}var nt=null;function jn(e){if(nt={x:e.clientX,y:e.clientY},X){te&&Math.hypot(e.clientX-te.x,e.clientY-te.y)>Yo&&(te=null),!te&&!X.pinned&&(Yn(X,e.clientX,e.clientY,Rt(e)),fe([...O])),V({x:e.clientX,y:e.clientY});return}re=dt(O,e.clientX,e.clientY),F=ge(e.clientX,e.clientY,U),V({x:e.clientX,y:e.clientY})}function Un(e){X&&(te?(X.locked=!X.locked,de=X.id,fe([...O])):(Xn(e.clientX,e.clientY)||e.clientX<Me||e.clientY<Me)&&_n(X),te=null,X=null,V({x:e.clientX,y:e.clientY}))}function Vn(e){if(e.button!==0)return;let t=ge(e.clientX,e.clientY,U);if(!t)return;let o=Xn(e.clientX,e.clientY);if(o){Ge(e),te=null,X=Kn(o,e.clientX,e.clientY,Rt(e)),V({x:e.clientX,y:e.clientY});return}let n=dt(O,e.clientX,e.clientY);if(n){Ge(e),Ae(),de=n.id,X=n,te={x:e.clientX,y:e.clientY},V({x:e.clientX,y:e.clientY});return}Ge(e),ue?.closeHelp(),E=[t],F=t,J?.show(t,Gt(),At()),V({x:e.clientX,y:e.clientY})}function qn(e){let t=ge(e.clientX,e.clientY,U);if(!t)return;Ge(e),ue?.closeHelp();let o=E.findIndex(r=>r.el===t.el);E=o>=0?E.filter((r,i)=>i!==o):[...E,t],F=t;let n=E[E.length-1];n?J?.show(n,Gt(),At()):J?.hide(),V({x:e.clientX,y:e.clientY})}function Jn(e){ge(e.clientX,e.clientY,U)&&Ge(e)}function Qn(e){ge(e.clientX,e.clientY,U)&&Ge(e)}function Ge(e){e.preventDefault(),e.stopPropagation()}function Dn(e,t){return e.left===t.left&&e.top===t.top&&e.width===t.width&&e.height===t.height}var Pn=0,Fn=0;function Zn(){ot=requestAnimationFrame(Zn);let t=E.filter(s=>s.el.isConnected).map(s=>_e(s.el)),o=F&&F.el.isConnected?_e(F.el):null;if(!(scrollX!==Pn||scrollY!==Fn||t.length!==E.length||t.some((s,c)=>!Dn(s,E[c]))||F===null!=(o===null)||F!==null&&o!==null&&!Dn(F,o)))return;Pn=scrollX,Fn=scrollY,E=t,F=o;let i=E[E.length-1],a=Vo();a!==Hn&&(Hn=a,i?J?.show(i,Gt(),At()):J?.hide()),V()}var Hn="";function Vo(){let e=E[0];return e?E.map(t=>[t.label,Math.round(t.width*100),Math.round(t.height*100),Math.round((t.left-e.left)*100),Math.round((t.top-e.top)*100)].join(",")).join(";"):""}function eo(){A?.resize()}function qo(){In||(In=!0,O=Nn().map(e=>({...e,id:Wn++}))),!A&&(bn(),A=Cn(),J=kn(A.root),ue=Sn(A.root,oe),ze=Mn(A.root),ue.update(0,{rulers:ve,xray:we,grid:Le,pixels:Re,freeze:Ze(),type:!1,panel:!1}),addEventListener("mousemove",jn),addEventListener("mousedown",Vn,{capture:!0}),addEventListener("mouseup",Un,{capture:!0}),addEventListener("click",Jn,{capture:!0}),addEventListener("auxclick",Qn,{capture:!0}),addEventListener("contextmenu",qn,{capture:!0}),addEventListener("resize",eo),ot=requestAnimationFrame(Zn),V())}function Lt(){removeEventListener("mousemove",jn),removeEventListener("mousedown",Vn,{capture:!0}),removeEventListener("mouseup",Un,{capture:!0}),removeEventListener("click",Jn,{capture:!0}),removeEventListener("auxclick",Qn,{capture:!0}),removeEventListener("contextmenu",qn,{capture:!0}),removeEventListener("resize",eo),cancelAnimationFrame(ot),ot=0,ue?.destroy(),ze?.destroy(),ze=null,we&&(we=!1,Tt(!1)),Et(!1),ue=null,J?.destroy(),J=null,A?.destroy(),A=null,wn(),F=null,E=[],X=null,te=null,re=null}function zn(e){if(Ko(e))e.preventDefault(),A?Lt():qo();else if(A&&nt&&(e.key.toLowerCase()===U.guideKeys.vertical||e.key.toLowerCase()===U.guideKeys.horizontal)){e.preventDefault();let t=e.key.toLowerCase()===U.guideKeys.vertical?"x":"y";Kn(t,nt.x,nt.y,Rt(e)),V()}else if(A&&(e.key==="Delete"||e.key==="Backspace"))e.preventDefault(),e.shiftKey?(O.some(t=>!t.pinned)&&Ae(),fe(O.filter(t=>t.pinned)),re=null,X=null,te=null,O.some(t=>t.id===de)||(de=null)):re&&_n(re),V();else if(A&&e.key.startsWith("Arrow")){let t=On(),o=e.key==="ArrowLeft"||e.key==="ArrowRight"?"x":"y";if(!t||t.axis!==o||(e.preventDefault(),t.pinned))return;Ae(`nudge:${t.id}`);let n=e.shiftKey?10:1;t.at+=e.key==="ArrowLeft"||e.key==="ArrowUp"?-n:n,t.caught="",fe([...O]),V()}else if(A&&e.key.toLowerCase()==="g"){e.preventDefault(),oe("grid");return}else if(A&&e.key.toLowerCase()==="k"){e.preventDefault(),oe("pixels");return}else if(A&&e.key.toLowerCase()==="f"){e.preventDefault(),oe("freeze");return}else if(A&&e.key.toLowerCase()==="x"){e.preventDefault(),oe("xray");return}else if(A&&e.key.toLowerCase()==="p"){e.preventDefault(),oe("pick");return}else if(A&&e.key.toLowerCase()==="t"){e.preventDefault(),oe("type");return}else if(A&&e.key.toLowerCase()==="c"){e.preventDefault(),oe("copy");return}else if(A&&e.key.toLowerCase()==="l"){let t=On();if(!t)return;e.preventDefault(),Ae(),t.pinned=!t.pinned,fe([...O]),V()}else if(A&&(e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==="z"){if(Te.depth()===0)return;e.preventDefault(),oe("undo");return}else if(A&&e.key.toLowerCase()===U.rulerKey){e.preventDefault(),oe("rulers");return}else if(A&&e.key.toLowerCase()===U.panelKey){e.preventDefault(),oe("panel");return}else if(e.key==="Escape"&&A){if(ze?.close()||ue?.closeHelp())return;E.length?(E=[],J?.hide(),V()):Lt()}}function Br(e={}){if(typeof window>"u"||window.__align)return;window.__align=!0,U=an(e),addEventListener("keydown",zn,{capture:!0});let t=import.meta.hot;t&&t.dispose(()=>{Lt(),removeEventListener("keydown",zn,{capture:!0}),delete window.__align})}export{Br as initAlign};
